                         ...
                         if (permitir = 'OK') or ( (permitir = 'EXISTE') and ( (docGuardado = umaContaPagar.getDocumento) and (dataGuardada = datetostr(umaContaPagar.getDataVencimento)) )  ) then
                           begin
                                incluido :=  umController.getControllerContaPagar.salvaContaPagar(umaContaPagar);
                                     if incluido = 'OK' then
                                        if umaContaPagar.getCodigo = 0 then
                                          ShowMessage('Conta incluída com sucesso!')
                                        else
                           ...
                           begin
                             ShowMessage('Conta a pagar já cadastrada!');
                     ...
                     permitir := 'COMPRA';
                              ...
                              if permitir = 'OK' then
                                  begin
                                   ...
                                   ShowMessage(QuotedStr(umaContaPagar.getDocumento) + ' excluída com sucesso!')
                                  end

                              else if permitir = 'NAO' then
                                  Showmessage('Não é possível excluir uma conta já paga!')
                              else
                                  ShowMessage('Não é possível excluir uma conta gerada através de compra!');
                inherited;
                end;
            end;
end;
...
begin
      Result:= False;
        if (self.edt_documento.Text = '') then
            ...
            begin
              ShowMessage('A conta para pagamento é obrigatório!');
            end
        else
            Result:= True;
end;
                    ...
                    begin
                      ...
                      if umaContaPagar.getumaForma.getCodigo = 2 then
                       begin
                        ...
                        umController.getControllerFluxo.SalvarFluxo('SAÍDA',umaContaPagar.getValor);
                       end;
