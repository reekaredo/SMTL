...
begin
      if self.btn_salvar.Caption = '&Salvar' then
       begin
            if validaCampos = True then
                begin
                    ...
                    incluido := umController.getControllerPais.salvaPais(umPais);
                       if incluido = 'OK' then
                          if umPais.getCodigo = 0 then
                            ShowMessage(umPais.getPais + ' incluído com sucesso!')
                          else
          ...
          begin
               men := CreateMessageDialog('Deseja mesmo excluir o país ' + QuotedStr(umPais.getPais) + '?',mtCustom,[mbYes,mbAbort]);
                          ...
                          if permitir = 'OK' then
                              begin
                               excluido :=  umController.getControllerPais.excluiPais(umPais);
                                    ShowMessage(QuotedStr(umPais.getPais) + ' excluído com sucesso!')
                              end
                          else
                              ShowMessage('Não foi possível excluir! Há estados vinculados a esse país');
                  inherited;
                end;
