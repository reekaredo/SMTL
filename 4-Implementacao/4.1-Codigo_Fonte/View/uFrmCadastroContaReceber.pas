                     ...
                     //  umaContaPagar.setDataPagamento(data_pagamento.DateTime);
                       ...
                       permitir := umController.getControllerContaReceber.pesquisaSalvar(umaContaReceber.getDocumento,dataV);
                         ...
                         if (permitir = 'OK') or ( (permitir = 'EXISTE') and ( (docGuardado = umaContaReceber.getDocumento) and (dataGuardada = datetostr(umaContaReceber.getDataVencimento)) )  ) then
                           ...
                           begin
                             ShowMessage('Conta a receber já cadastrada!');
                     ...
                     permitir := 'VENDA';
                              ...
                              if permitir = 'OK' then
                                  begin
                                   ...
                                   ShowMessage(QuotedStr(umaContaReceber.getDocumento) + ' excluída com sucesso!')
                                  end

                              else if permitir = 'NAO' then
                                  Showmessage('Não é possível excluir uma conta já paga!')
                              else
                                  ShowMessage('Não é possível excluir uma conta gerada através de venda!');
                inherited;
                end;
            end;
end;
...
begin
      Result:= False;
        if (self.edt_documento.Text = '') then
            ...
            begin
              ShowMessage('O cliente é obrigatório!');
