...
begin
        if Self.btn_salvar.Caption = '&Salvar' then
           begin
                ...
                begin
                   ShowMessage('O nome da marca é obrigatório!');
                  ...
                  begin
                      ...
                      incluido := umController.getControllerMarca.salvaMarca(umaMarca);
                         if incluido = 'OK' then
                            if umaMarca.getCodigo = 0 then
                              ShowMessage(umaMarca.getNome + ' incluída com sucesso!')
                            else
                        ...
                        permitir := umController.getControllerMarca.pesquisaDependencia(umaMarca.getCodigo);
                          if permitir = 'OK' then
                             begin
                              ...
                              ShowMessage(QuotedStr(umaMarca.getNome) + ' excluída com sucesso!')
                             end
                          else
                             Showmessage('Não foi possível excluir a marca! Há produtos vinculados a ela!');
                    inherited;
                    end;
              end;
end;
